...
const GroupChat = () => {
  ...
  return (
    ...
    <div className=" fixed top-0 w-72 h-full bg-gray-600 flex flex-col items-center border-r border-gray-400">
      {/* <div className=" flex  items-center gap-56 py-3">
        <h1 className=" text-sm font-medium"> Online</h1>
        <p className=" text-[10px] font-light">12</p>
      </div> */}
      {/* User Status Online or not */}
      {/* <ScrollArea className="flex items-center gap-2">
        {[1, 2, 3, 4, 5,].map((el)=> (
            <Avatar>
                <AvatarImage src={faker.image.avatar()} alt="Users" width={50} height={50} className=" rounded-full" />
            </Avatar>
        ))}
      ...
      <div className=" flex  items-center gap-x-36 py-2">
        ...
        <div className=" text-[10px] font-light" onClick={ () => setOpenDialog(true)}>
          <Scaling size={18}/>
        </div>
      </div>
      <div className=" py-2">
