...
const ActionButton: FC<ActionButtonProps> = ({ Icon, label }) => (
  <div className='flex space-y-2 flex-col items-center w-fit px-4 py-2 rounded-xl bg-gray-600 dark:bg-gray-800'>
    <Icon />
    <p className='text-xs text-light-1'>{label}</p>
  </div>
);
...
export const GroupSheet: FC<GroupSheetProps> = ({ groupName, chatId, groupImage, admin }) => {
  ...
  const { useMutate: leaveGroup, state: leaveGroupState } = useMutationHandler(
    api.conversation.leaveGroup
  );
  const { useMutate: blockGroup, state: blockGroupState } = useMutationHandler(
  ...
  return (
    <ScrollArea className='h-full'>
      ...
      <SheetTitle className='text-center mt-2 text-2xl text-light-1'>{groupName}</SheetTitle>
          ...
          groupMembers.members.map(member => (
            ...
            >
              <div className='flex items-center space-x-3'>
                <Avatar>
                  <AvatarImage src={member.imageUrl! || member.username[0].slice(0,2)} />
